...
export class AdminRegisterConsumptionComponent {
  ...
  findPartner(): void {
    ...
    this.adminService.searchByIdentification(this.identificationInput.trim()).pipe(
      map(p => p),
      catchError(err => {
    ...
    ).subscribe(p => { this.partnerFound = p; this.searchingPartner = false; });
  }
  ...
  submit(): void {
    ...
    this.adminService.registerConsumption(req).pipe(
      catchError(err => {
